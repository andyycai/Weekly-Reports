...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 16 08:00:00 2025
...
class NSQ_Model(mesa.Model):
    ...
    def __init__(
            ...
            beta_S_to_Q_due_to_N = 0.05623,
            beta_S_to_Q_due_to_Q = 0.35214,
            beta_Q_to_S_due_to_S = 0.01334,
            # Initial population parameters (%, SMOKER, QUITTER):
            initial_SMOKER_pct = 0.456,
            initial_QUITTER_pct = 0.166,
...
model_fc = NSQ_Model(network_type = nx.complete_graph, n = 1000)
model_fc.run_model(50)
model_fc.print_pct_data()

model_er = NSQ_Model(network_type = nx.erdos_renyi_graph, n = 1000, p = 0.003)
model_er.run_model(50)
model_er.print_pct_data()

model_ba = NSQ_Model(network_type = nx.barabasi_albert_graph, n = 1000, m = 2)
model_ba.run_model(50)
model_ba.print_pct_data()

model_ws = NSQ_Model(network_type = nx.watts_strogatz_graph, n = 1000, k = 4, p = 0.3)
model_ws.run_model(50)
model_ws.print_pct_data()

model_lfr = NSQ_Model(network_type = nx.LFR_benchmark_graph, 
                      n = 1000, tau1 = 2.5, tau2 = 1.5, mu = 0.6, average_degree = 3)
model_lfr.run_model(50)
model_lfr.print_pct_data()
